...
namespace UnityEditor.Purchasing
{
    ...
    internal class ObfuscationGenerator
    {
        ...
        private const string kAppleCertPath = "Editor/AppleIncRootCertificate.cer";
            ...
            ref string googleError, string googlePlayPublicKey)
        {
            try
            {
                // First things first! Obfuscate! XHTLOA!
                {
                    ...
                    try
                    {
                        string appleCertPath = UnityIapPluginHierarchy.Instance.GetPathWithRoot(kAppleCertPath);
                        byte[] bytes = System.IO.File.ReadAllBytes(appleCertPath);
