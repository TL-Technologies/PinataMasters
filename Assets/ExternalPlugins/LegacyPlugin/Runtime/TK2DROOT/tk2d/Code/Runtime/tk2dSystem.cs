    ...
    public static string CurrentPlatform
    {
        get 
        {
            if (!currentPlatformInitialized)
            {
                ...
                {
                    #if UNITY_IOS
                    currentPlatform = (Screen.height > retinaTreshold || Screen.width > retinaTreshold) ? (platform4X) : (platform2X);
                    #elif UNITY_ANDROID
                    // Custom logic of detecting platform using screen size and dpi
                    currentPlatform = (Screen.dpi > 239.999f && Screen.width > 1000 && Screen.height > 1000) ? (platform4X) : (platform2X);
                    #endif
                }
