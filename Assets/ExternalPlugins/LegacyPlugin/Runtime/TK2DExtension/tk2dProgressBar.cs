...
public class tk2dProgressBar : tk2dSprite 
{
	...
	void BuildMesh()
	{
		...
		tempIndexes = tempIndexes.EnsureLength(quads * 6);
	...
	public void OnFill(ref Vector3[] verts_array, ref Vector2[] uvs_array, ref Color[] cols_array, float curFillAmount) 
	{
		...
		verts_array = verts_array.EnsureLength(4, !is4VertexType);
		uvs_array = uvs_array.EnsureLength(4, !is4VertexType);
		...
		else
		{
			...
			tempOXY = tempOXY.EnsureLength(4);
			tempOUV = tempOUV.EnsureLength(4);
			...
			verts_array = verts_array.EnsureLength(tempVertexList.Count);
			uvs_array = uvs_array.EnsureLength(tempUVList.Count);
		...
		cols_array = cols_array.EnsureLength(verts_array.Length);
