...
public class GUICellColliderSetter : MonoBehaviour , ILayoutCellHandler
{
	...
	public void RepositionForCell (LayoutCellInfo info)
	{
		if (rePositionPlace)
		{
			float y = (rePositionPlace.transform.position.y - transform.position.y) / (Vector3.one / ((float)ScreenDimensions.Height / (float)Screen.height)).y;
			float x = (rePositionPlace.transform.position.x - transform.position.x) / (Vector3.one / ((float)ScreenDimensions.Width / (float)Screen.width)).x;

			Vector3 center = new Vector3 (x, y, 0);
			ToReposition.center = center;
			ToReposition.size = new Vector3 (rePositionPlace.ReceivedRect.size.x * sizeMultiplier, rePositionPlace.ReceivedRect.size.y * sizeMultiplier, 1);
		}
		else
		{
