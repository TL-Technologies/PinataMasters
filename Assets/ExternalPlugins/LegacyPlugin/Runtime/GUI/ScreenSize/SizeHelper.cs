...
{		
	...
	static public float HeightFactor 
    {
		get
        {
			if (heightFactor < 0f) 
            {
                heightFactor = ScreenDimensions.Height / (float) SizeHelperSettings.Instance.baseHeight;
			}
			return heightFactor;
		}
	}
	...
	static public float WidthFactor 
    {
		get 
        {
			if (widthFactor < 0f) 
            {
                widthFactor = ScreenDimensions.Width / (float) SizeHelperSettings.Instance.baseWidth;
			}
			return widthFactor;
		}
	}
	...
	static Vector3 RecalculateByFit(Vector3 baseValue, Fit type, bool half) 
    {
		if (type == Fit.Height) 
        {
            baseValue.y = half ? (ScreenDimensions.Height / 2f) : ScreenDimensions.Height;
		} 
        else if (type == Fit.Width) 
        {
            baseValue.x = half ? (ScreenDimensions.Width / 2f) : ScreenDimensions.Width;
		} 
