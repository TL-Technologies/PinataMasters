...
namespace Modules.Legacy.TextureManagement.Supply
{
	public class tmUtility
	{
		...
		{
			tmResourceCollectionLink[]
				allLinks = Resources.LoadAll<tmResourceCollectionLink>(tmSettings.ResourceFolder);
		...
		public static List<tmResourceCollectionLink> GetAllResourceLinksFor(tmPlatform platform,
			List<tmResourceCollectionLink> _links)
		{
			HashSet<string> allCollectionsGuids = new HashSet<string>();
		...
		public static void ValidateMesh(Mesh mesh)
		{
			...
			if (mesh != null)
			{
				...
				if (outOfRange)
				{
					CustomDebug.LogError("WRONG MESH UV : " + UnityEditor.AssetDatabase.GetAssetPath(mesh) + "/" +
					                     mesh.name);
				}
			}
			#endif
