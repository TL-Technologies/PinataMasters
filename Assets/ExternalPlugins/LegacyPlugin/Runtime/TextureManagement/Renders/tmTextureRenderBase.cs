...
namespace Modules.Legacy.TextureManagement.Renders
{
	...
	public class tmTextureRenderBase : MonoBehaviour, tmIRebuildable
	{
		...
		{
			get { return mainTexCollectionGUID; }
			set
			{
		...
		public string MainTextureName
		{
			...
			set { MainTextureID = MainTexCollection.GetTextureDefenitionByName(value).textureGuid; }
		}
		...
		{
			get { return lightmapCollectionGUID; }
			set
			{
		...
		protected virtual void BuildMesh()
		{
			if (Mesh != null && !string.IsNullOrEmpty(MainTextureID) && MainTexCollection != null)
			{
				...
				if (mainTextureDefenition != null)
				{
					...
					if ((ModifiedFlag & (ModifiedFlags.ModifiedMesh | ModifiedFlags.ModifiedUV1 |
					                     ModifiedFlags.ModifiedUV2)) != 0)
					{
						tmManager.Instance.MeshInstance(Mesh, out sharedMesh, mainTextureDefenition,
							lightmapTextureDefenition, false);
		...
		protected virtual Material MaterialInstance(Material original, tmTextureCollectionPlatform mainCollection,
			tmTextureCollectionPlatform lightmapCollection)
		{
