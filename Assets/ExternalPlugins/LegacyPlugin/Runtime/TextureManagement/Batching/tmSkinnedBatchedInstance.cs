...
namespace Modules.Legacy.TextureManagement.Batching
{
	public class tmSkinnedBatchedInstance : tmBatchedInstance
	{
		...
		protected override void PostRecombine(CombineInstance[] combos)
		{
			...
			tempBones = tempBones.EnsureLength(bones);
			tempBindPoses = tempBindPoses.EnsureLength(bones);
			tempBoneWeights = tempBoneWeights.EnsureLength(Mesh.vertexCount);
			...
			for (int partIndex = 0, partsCount = BatchedParts.Count; partIndex < partsCount; partIndex++)
			{
				...
				if (partSMR)
				{
					...
					Matrix4x4 partToBatchMatrix =
						part.CachedTransform.worldToLocalMatrix * CachedTransform.localToWorldMatrix;
