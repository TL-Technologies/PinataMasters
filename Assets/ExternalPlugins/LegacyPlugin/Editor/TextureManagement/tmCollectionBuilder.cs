	...
	public static class tmCollectionBuilder
	{
		public static void BuildCollection(tmTextureCollection collection)
		{
			if (collection != null)
			{
					...
					if (texture != null)
					{
						...
						if (textureRef == null)
						{
							...
							textureRef.textureName =
								tmUtility.PlatformlessPath(texture.name); // should attach to name ? or guid better
				...
				collection =
					tmEditorUtility.GUIDToAsset(collection.assetGuid, typeof(tmTextureCollection)) as
						tmTextureCollection;
		...
		public static void BuildPlatformCollections(tmTextureCollection collection)
		{
			if (collection != null)
			{
				...
				{
					if (collection.forcedScale > 0 &&
					    Mathf.Abs(platform.scale - collection.forcedScale) > float.Epsilon)
					{
						continue;
					}

					tmTextureCollectionPlatformRef collectionPlatformRef =
						collection.platforms.Find(f => (f != null) && f.platform.Equals(platform));
					...
					{
						collectionPlatform =
							tmCollectionBuilder.CreatePlatformCollectionFromMainCollection(collection, platform);
					}
		...
		public static tmTextureCollectionPlatform CreatePlatformCollectionFromMainCollection(
			tmTextureCollection collection, tmPlatform platform)
		{
			...
			tmTextureCollectionPlatform collectionPlatform =
				AssetDatabase.LoadAssetAtPath(platformPath, typeof(tmTextureCollectionPlatform)) as
					tmTextureCollectionPlatform;
		...
		public static void UpdatePlatformCollection(tmTextureCollectionPlatform collectionPlatform,
			List<Texture2D> originalTextures)
		{
			...
			foreach (var texture in originalTextures)
			{
				if (texture != null)
				{
					tmTextureDefenition textureRef =
						collectionPlatform.textureDefenitions.Find(f => f.texture == texture);
					if (textureRef == null)
					{
						textureRef = new tmTextureDefenition();
						textureRef.texture = texture;
					...
					Texture2D asset =
						PlatformSpecifiedTexture(textureRef.texture, collectionPlatform.platform, rebuild);
					if (asset != null)
					{
			...
			collectionPlatform.textureDefenitions.Sort((a, b) =>
				(string.Compare(a.textureName, b.textureName, System.StringComparison.OrdinalIgnoreCase)));
			...
			collectionPlatform =
				tmEditorUtility.GUIDToAsset(collectionPlatform.assetGuid, typeof(tmTextureCollectionPlatform)) as
					tmTextureCollectionPlatform;
...
//			asset.Apply();
				...
				asset = tmEditorUtility.RescaleTexture(mainTexture,
					platform.scale / tmSettings.Instance.DefaultPlatform.scale);
		...
		public static void BuildAtlas(tmTextureCollectionPlatform collectionPlatform)
		{
			...
			Texture2D atlas = new Texture2D(4096, 4096, collectionPlatform.importSettings.format,
				collectionPlatform.importSettings.generateMipMaps);
			...
			Rect[] spritesRect =
				atlas.PackTextures(textures.ToArray(), collectionPlatform.importSettings.border, 4096, false);
			...
			{
				TextureCache.TextureCache.UnloadTexture(collectionPlatform.Atlas);
		...
		public static void BuildCollectionsForModifiedAssets(string[] importedAssets)
		{
			...
			foreach (string path in importedAssets)
			{
				...
				if (index != null)
				{
					tmTextureCollection coll =
						tmEditorUtility.GUIDToAsset(index.assetGUID,
							typeof(tmTextureCollection)) as tmTextureCollection;
		...
		public static void CreatePlatformResourceLink(tmTextureCollectionPlatform collectionPlatform)
		{
			...
			string assetPath = tmSettings.AssetsFolder + tmSettings.ResourceLinkPrefix +
			                   collectionPlatform.collectionGuid + ".asset";
			string resourcePath = tmSettings.ResourceFolder + tmSettings.ResourceLinkPrefix +
			                      collectionPlatform.collectionGuid;

			tmResourceCollectionLink link =
				Resources.Load(resourcePath, typeof(tmResourceCollectionLink)) as tmResourceCollectionLink;
			if (link == null)
			{
		...
		{
			List<tmResourceCollectionLink> resourceLink =
				AssetUtility.GetAssetsAtPath<tmResourceCollectionLink>("tmResources");
		...
		public static void UpdateResourceLink(tmResourceCollectionLink link)
		{
			...
			if (tmSettings.Instance.TargetPlatform.name.Equals("iPhone") &&
			    link.collection.platform.name.Equals("iPad Retina"))
			{
