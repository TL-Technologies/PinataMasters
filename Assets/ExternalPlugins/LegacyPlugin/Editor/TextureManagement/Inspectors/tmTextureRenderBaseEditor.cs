...
namespace Modules.Legacy.TextureManagement.Editor.Inspectors
{
	...
	public class tmTextureRenderBaseEditor : UnityEditor.Editor
	{
		tmTextureRenderBase render
		{
			get { return target as tmTextureRenderBase; }
		}
...
//				renderer.hideFlags = HideFlags.None;
		...
		public override void OnInspectorGUI()
		{
			...
			if (render.Mesh != null && render.Mesh.uv2.Length > 0)
			{
...

				{
					...
					EditorGUI.showMixedValue = mixed;
					{
						...
						if (CollectionPopup(isAOMaterial ? "AO Collection" : "Lightmap Collection",
							ref lightmapCollectionGUID))
						{
							foreach (tmTextureRenderBase r in targets)
							{
				...
				}
				{
					...
					EditorGUI.showMixedValue = mixed;
					{
						...
						if (TexturePopup(isAOMaterial ? "AO Texture" : "Lightmap Texture", render.LightmapCollection,
							ref lightmapTextureID))
						{
							foreach (tmTextureRenderBase r in targets)
							{
			...
			render.ModifiedFlag =
				(tmTextureRender.ModifiedFlags) EditorGUILayout.EnumFlagsField("ModifiedFlags", render.ModifiedFlag);
		...
		static bool TexturePopup(string label, tmTextureCollectionBase collection, ref string texureID)
		{
			...
			if (entryIndex != -1)
			{
				...
				else
				{
					...
					EditorGUI.DrawTextureTransparent(rect, def.texture, ScaleMode.ScaleToFit,
						def.texture == null ? 1f : (def.texture.width * 1f / def.texture.height));
