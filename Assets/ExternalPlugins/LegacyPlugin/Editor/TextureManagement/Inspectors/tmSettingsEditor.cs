...
namespace Modules.Legacy.TextureManagement.Editor.Inspectors
{
	...
	public class tmSettingsEditor : UnityEditor.Editor
	{
		tmSettings settings
		{
			get { return target as tmSettings; }
		}
		...
		public override void OnInspectorGUI()
		{
			...
			settings.ForceStaticGeometry =
				EditorGUILayout.Toggle("Force Static Geometry", settings.ForceStaticGeometry);
			...
			settings.isImmediateTextureLoadEnabled = EditorGUILayout.Toggle("Synchronous texture load enabled",
				settings.isImmediateTextureLoadEnabled);
			settings.isAtlasesPreloadEnabled = EditorGUILayout.Toggle("Atlases preload on tmManager's Awake enabled",
				settings.isAtlasesPreloadEnabled);
			settings.isAtlasesUnloadEnabled = EditorGUILayout.Toggle("Atlases unload on ref count = 0 enabled",
