...
namespace Modules.Legacy.TextureManagement.Editor
{
	...
	public class tmIndex : ScriptableSingleton<tmIndex>
	{
		...
		[SerializeField]
		List<tmTextureCollectionIndex> texturePlatformCollections = new List<tmTextureCollectionIndex>();
		#if UNITY_EDITOR
		...
		{
			get { return textureCollections; }
		}


		public List<tmTextureCollectionIndex> TexturePlatformCollections
		{
			get { return texturePlatformCollections; }
		}
		...
		{
			tmTextureCollectionIndex index =
				collections.Find(f => f.textureCollectionGUID.Equals(collection.collectionGuid));
			if (index == null)
			{
			...
			collection.textureDefenitions.Sort((a, b) =>
