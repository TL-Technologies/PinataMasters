		...
		public static void RebuildIndex()
		{
			...
			for (int i = 0; i < collections.Count; i++)
			{
				...
				if (!collection.IsNull())
				{
					...
					try
					{
						EditorUtility.DisplayProgressBar("Texture Manager", "building : " + collection.name,
							current++ * 1f / collections.Count);
...
//		tmCollectionBuilder.ValidateResourceLinks();
			...
			for (int i = 0; i < target.GetComponent<Renderer>().sharedMaterials.Length; i++)
			{
				...
				if (main != null)
				{
					...
					if (inst != null)
					{
						...
						render.MainTextureID =
							render.MainTexCollection.GetTextureDefenitionByName(main.name).textureGuid;
					}
				}
				...
				if (lightmap != null)
				{
					...
					if (inst != null)
					{
						...
						render.LightmapTextureID = render.LightmapCollection.GetTextureDefenitionByName(lightmap.name)
							.textureGuid;
					}
				}
		...
		{
			tmIndex.Instance.staticPrefabs.Sort((a, b) =>
