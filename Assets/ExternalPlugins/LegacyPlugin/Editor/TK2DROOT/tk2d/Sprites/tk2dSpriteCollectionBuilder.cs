...
{
    public const string FOLDER_STREAMING_TEXTURES = "Assets/StreamingAssets/tk2dTextures/";
...
//            }
        ...
        if (gen.usePosterize)
        {
            CustomDebug.Log("Start Posterize for: " + texturePath);
            string posterizeScriptPath = UnityPath.Combine(
	            LegacyPluginHierarchy.Instance.RootPath,
	            "Editor",
	            "PngPostProcess",
	            "posterize.py");
            if (!string.IsNullOrEmpty(posterizeScriptPath))
            {
                System.Diagnostics.Process p = new System.Diagnostics.Process();
                p.StartInfo.Arguments = "'" + posterizeScriptPath + "' '" + texturePath + "' " + gen.posterizeValue;
            ...
            {
                CustomDebug.LogError("Posterize script not found!");
                pngCrushPostProcessAction();
            }
        }
        else
    ...
    public static bool Rebuild(tk2dSpriteCollection gen)
    {
        ...
        System.Array.Resize(ref gen.atlasWidthTexture, atlasData.Length);
        System.Array.Resize(ref gen.atlasHeightTexture, atlasData.Length);
		...
		for (int atlasIndex = 0; atlasIndex < atlasData.Length; ++atlasIndex)
		{
            if (gen.freeSizeAtlas) 
            {
                ...
                for (int i = 0; i < atlasData[atlasIndex].entries.Length; ++i) 
                {
                    ...
                    int currX = entry.x + source.width;
                    int currY = entry.y + source.height;

                    if (entry.flipped) 
                    {
                        currX = entry.x + source.height;
                        currY = entry.y + source.width;
                    }
                ...
                atlasData[atlasIndex].width = width;
                atlasData[atlasIndex].height = height;
            }
            ...
            gen.atlasWidthTexture[atlasIndex] = tex.width;
            gen.atlasHeightTexture[atlasIndex] = tex.height;
			...
			{
				texturePath = FOLDER_STREAMING_TEXTURES + gen.name + atlasIndex + ".png";
			}
        ...
        coll.widthTextures = new int[gen.atlasWidthTexture.Length];
        for (int i = 0; i < gen.atlasWidthTexture.Length; ++i)
        {
            coll.widthTextures[i] = gen.atlasWidthTexture[i];
        }

        coll.heightTextures = new int[gen.atlasHeightTexture.Length];
        for (int i = 0; i < gen.atlasHeightTexture.Length; ++i)
        {
            coll.heightTextures[i] = gen.atlasHeightTexture[i];
        }
        ...
        gen.spriteCollection.textureLoadAsync = gen.asyncLoadEnabled;
	...
	static void SetUpTargetTexture(tk2dSpriteCollection gen, Texture2D tex)
	{
        ...
        {
            case tk2dSpriteCollection.TextureCompression.Uncompressed: targetFormat = TextureImporterFormat.RGBA32; break;
            case tk2dSpriteCollection.TextureCompression.Reduced16Bit: targetFormat = TextureImporterFormat.RGBA16; break;
            case tk2dSpriteCollection.TextureCompression.Dithered16Bit_Alpha: targetFormat = TextureImporterFormat.RGBA16; break;
            case tk2dSpriteCollection.TextureCompression.Dithered16Bit_NoAlpha: targetFormat = TextureImporterFormat.RGBA16; break;
            case tk2dSpriteCollection.TextureCompression.Compressed: targetFormat = TextureImporterFormat.RGBA32; targetCompression = TextureImporterCompression.CompressedHQ; break;
            case tk2dSpriteCollection.TextureCompression.Alpha8: targetFormat = TextureImporterFormat.Alpha8; break;

            default: targetFormat = TextureImporterFormat.RGBA32; break;
        }
        ...
        if (targetFormat != defaultPlatform.format)
        {
            defaultPlatform.format = targetFormat;
            textureDirty = true;
        }
	...
	static void UpdateFontData(tk2dSpriteCollection gen, float scale, tk2dEditor.Atlas.Data[] packers, List<SpriteLut> spriteLuts, tk2dSpriteCollectionFont font, tk2dEditor.Font.Info fontInfo)
	{
        ...
        float newScale = (gen.enablePlatformScaleVertex) ? (scale) : (1);      
        tk2dEditor.Font.Builder.BuildFont(fontInfo, font.data, newScale, font.charPadX, font.dupeCaps, font.flipTextureY, font.gradientTexture, font.gradientCount);
	}
    ...
    static void UpdateVertexCache(tk2dSpriteCollection gen, float scale, tk2dEditor.Atlas.Data[] packers, tk2dSpriteCollectionData coll, List<SpriteLut> spriteLuts)
    {
        for (int i = 0; i < sourceTextures.Length; ++i)
        {
            ...
            else
            {
				...
				{
                    float newScale = (gen.enablePlatformScaleVertex) ? (1) : (scale);
                    coll.spriteDefinitions[i].positions[j] = positions[j] / newScale;
			...
			else
			{
                ...
                float newScale = (gen.enablePlatformScaleVertex) ? (1) : (scale);
                coll.spriteDefinitions[i].untrimmedBoundsData[0] = (boundsMax + boundsMin) / 2.0f / newScale;
                coll.spriteDefinitions[i].untrimmedBoundsData[1] = (boundsMax - boundsMin) / newScale;
			}
