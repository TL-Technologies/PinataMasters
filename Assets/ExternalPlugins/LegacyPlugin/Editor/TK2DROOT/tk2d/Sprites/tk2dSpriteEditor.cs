...
public class tk2dSpriteEditor : Editor
{
    ...
    protected void DrawSpriteEditorGUI()
    {
        ...
        if (targetSprites[0].Collection != null)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Layout setting:");
            bool lockedLayoutSize = EditorGUILayout.Toggle("Is layout size locked?", targetSprites[0].isLayoutSizeLocked);

            if (lockedLayoutSize != targetSprites[0].isLayoutSizeLocked)
            {
                foreach (tk2dBaseSprite s in targetSprites)
                {
                    s.isLayoutSizeLocked = lockedLayoutSize;
                }
            }
            EditorGUILayout.EndHorizontal();
            ...
            bool isRoundShiftDisabled = EditorGUILayout.Toggle("Is round shift disabled?", targetSprites[0].isRoundShiftDisabled);

            if (isRoundShiftDisabled != targetSprites[0].isRoundShiftDisabled)
            {
                foreach (tk2dBaseSprite s in targetSprites)
                {
                    s.isRoundShiftDisabled = isRoundShiftDisabled;
                }
            }
