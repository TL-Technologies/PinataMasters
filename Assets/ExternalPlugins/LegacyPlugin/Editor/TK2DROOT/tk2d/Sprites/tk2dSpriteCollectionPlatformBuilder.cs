...
namespace tk2dEditor.SpriteCollectionBuilder
{
	public static class PlatformBuilder
	{
			...
			if (!root)
			{
				...
				foreach (tk2dSpriteCollectionDefinition param in proxy.textureParams)
				{
					...
					else
					{
						...
						if (tex == null)
						{
							CustomDebug.LogError("Unable to load platform specific texture '" + platformTexture + "'");
						}
						else
						{
							param.texture = tex;
		...
		{
			CustomDebug.LogError(string.Format("Unable to find platform specific {0} '{1}' for platform '{2}'", assetType, assetName, platformName));
		}
