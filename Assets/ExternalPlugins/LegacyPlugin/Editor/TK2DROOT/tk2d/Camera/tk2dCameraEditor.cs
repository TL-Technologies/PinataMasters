	...
	public class tk2dCameraSceneGUI
	{
		...
		void PreviewWindowFunc(int windowId) 
		{
			...
			switch (Event.current.type)
			{
				...
				{
                    Vector2 sizeRect = new Vector2(rs.width, rs.height) * multiplySizeViewToPixels;
                    Vector2 positionRect = new Vector2(previewWindowRect.x + widthTweak + rs.x, (previewWindowRect.y + rs.height + heightTweak) - rs.y) * multiplySizeViewToPixels;
                    ...
                    previewCamera.pixelRect = new Rect(positionRect.x, Camera.current.pixelHeight - positionRect.y, sizeRect.x, sizeRect.y);
		...
		public void OnSceneGUI(tk2dCamera target)
		{
            ...
            sizeSceneView = new Vector2(Screen.width, Screen.height);
            if (SceneView.currentDrawingSceneView != null)
            {
                sizeSceneView = SceneView.currentDrawingSceneView.position.size;
            }
            multiplySizeViewToPixels = Camera.current.pixelWidth / sizeSceneView.x;

            float maxHeight = sizeSceneView.y / 5;
			...
			int viewportOffsetBottom = 10;
            previewWindowRect = new Rect(viewportOffsetLeft, sizeSceneView.y - height - viewportOffsetBottom, width, height);
			Handles.BeginGUI();
        ...
        Vector2 sizeSceneView;
        Vector2 sizeCameraPixels; 
        float multiplySizeViewToPixels = 1;
	}	
}
